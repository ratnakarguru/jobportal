...
function JobsPage() {
  ...
  useEffect(() => {
    ...
    if (cachedUser) {
      try {
        ...
        console.error("Error decoding local user data context payload:", e);
      }
    }
  }, []);
  ...
  // Perform UI operations (Filtering, Searching, & Sorting) dynamically
  useEffect(() => {
    let result = [...jobs];
    ...
    if (sortBy === "salary") {
      result.sort((a, b) => {
        const valA = parseInt(String(a.salary || "").replace(/[^0-9]/g, "")) || 0;
        const valB = parseInt(String(b.salary || "").replace(/[^0-9]/g, "")) || 0;
        return valB - valA;
      });
    }
            ...
            {isLoading ? (
              <div className="text-center p-5">
                <div className="spinner-border text-primary" role="status">
                  <span className="visually-hidden">Loading database openings...</span>
                </div>
              </div>
            ) : filteredJobs.length === 0 ? (
              ...
              <div className="modal-header bg-dark text-white p-4 align-items-start border-0 position-relative">
                <div className="d-flex align-items-center mt-2">
                  <div className="bg-white bg-opacity-15 text-white rounded-3 d-flex align-items-center justify-content-center fw-bold me-3" style={{ width: "60px", height: "60px", fontSize: "1.25rem" }}>
                    {selectedJob.logoText || (selectedJob.company ? selectedJob.company.substring(0,2).toUpperCase() : "JB")}
                  </div>
                  <div>
